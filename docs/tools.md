...
| `field_lineage` | Query compiled field-lineage artifacts. Returns matches across **every lineage family** the artifact carries: deterministic shared-copybook reuse, inferred cross-copybook candidates (high-confidence / ambiguous / semantic-inferred), CALL boundary field flow (caller USING ↔ callee LINKAGE), and DB2 cross-program data flow (writer/reader pairs with host variables, plus column-level `writerHostVar → column → readerHostVar` pairings). The response surfaces per-family arrays + counters + a `familyAvailability` map distinguishing "no artifact for family" from "family available but no match". The top-level `evidence` envelope is computed across all families — a CALL boundary or DB2 deterministic match alone is enough to ship `strong`/`deterministic`. DB2 `qualified_name` matching falls back to comparing the leaf segment against `dataItem.name`; the response includes a `notes` entry calling out the limitation. |
...
| `code_query field_lineage` | `strong` when ANY family produces a deterministic-tier match (copybook deterministic, CALL boundary deterministic, or DB2 entry). `weak` when only inferred matches exist across copybook high-confidence / CALL boundary high / semantic-inferred families. `absent` + `abstain` when only ambiguous matches or no match across all families. Mapped from COBOL lineage tiers via `cobolTierToEvidence`. |
